...
class Image_adapter(nn.Module):
    def __init__(self, K=6, hidden_size=1024):
        super().__init__()
        self.embeddings = nn.Parameter(torch.zeros(K, hidden_size))
...
def collate_fn(examples, with_prior_preservation=False):
    ...
    batch['index'] = torch.tensor([example['index'] for example in examples])
...
def main(args):
    ...
    for i in range(len(train_dataset)):
        ...
        with torch.no_grad():
            img_state = img_model.encode_image(clip_trans(pixel_values))
        img_adapter.embeddings.data[i] = img_state.view(-1)
        print(img_adapter.embeddings.data[i])
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            with accelerator.accumulate(unet):
                ...
                img_state = img_adapter.embeddings[batch['index']].reshape(-1, 1, 1024)
