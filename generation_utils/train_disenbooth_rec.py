...
class DreamBoothDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        image_noconcept_path = image_path.parent.parent / (image_path.parent.name + '_nosubject') / image_path.name
        instance_image = Image.open(image_path)
        instance_image = exif_transpose(instance_image)
        instance_image_noconcept = Image.open(image_noconcept_path)
        instance_image_noconcept = exif_transpose(instance_image_noconcept)

        if not instance_image.mode == "RGB":
            instance_image = instance_image.convert("RGB")
        if not instance_image_noconcept.mode == "RGB":
            instance_image_noconcept = instance_image_noconcept.convert("RGB")
        example["instance_images"] = self.image_transforms(instance_image)
        example["instance_images_noconcept"] = self.image_transforms(instance_image_noconcept)
...
def collate_fn(examples, with_prior_preservation=False):
    ...
    pixel_values_noconcept = [example["instance_images_noconcept"] for example in examples]
    ...
    pixel_values_noconcept = torch.stack(pixel_values_noconcept)
    pixel_values_noconcept = pixel_values_noconcept.to(memory_format=torch.contiguous_format).float()
    ...
    batch = {
        ...
        "pixel_values_noconcept": pixel_values_noconcept,
    }
...
def main(args):
    ...
    if accelerator.is_main_process:
        ...
        accelerator.init_trackers("disenbooth-original", config=tracker_config)
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            with accelerator.accumulate(unet):
                ...
                pixel_values_noconcept = batch["pixel_values_noconcept"].to(dtype=weight_dtype)

                if vae is not None:
                    # Convert images to latent space
                    model_input = vae.encode(pixel_values).latent_dist.sample()
                    model_input = model_input * vae.config.scaling_factor
                    model_input_noconcept = vae.encode(pixel_values_noconcept).latent_dist.sample()
                    model_input_noconcept = model_input_noconcept * vae.config.scaling_factor

                else:
                    model_input = pixel_values
                    model_input_noconcept = pixel_values_noconcept

                # Sample noise that we'll add to the latents
                noise = torch.randn_like(model_input)
                noise_noconcept = torch.randn_like(model_input_noconcept)
                ...
                noisy_model_input_noconcept = noise_scheduler.add_noise(model_input_noconcept, 
                                                                        noise_noconcept, timesteps)
                ...
                if accelerator.unwrap_model(unet).config.in_channels == channels * 2:
                    ...
                    noisy_model_input_noconcept = torch.cat([noisy_model_input_noconcept, 
                                                             noisy_model_input_noconcept], dim=1)
                ...
                img_pred = unet(noisy_model_input_noconcept, 
                                 timesteps, img_state, 
                ...
                if noise_scheduler.config.prediction_type == "epsilon":
                    target = noise
                    target_noconcept = noise_noconcept
                elif noise_scheduler.config.prediction_type == "v_prediction":
                    target = noise_scheduler.get_velocity(model_input, noise, timesteps)
                    target_noconcept = noise_scheduler.get_velocity(model_input_noconcept, 
                                                                    noise_noconcept, timesteps)
                else:
                    raise ValueError(f"Unknown prediction type {noise_scheduler.config.prediction_type}")
                main_loss = F.mse_loss(model_pred.float(), target.float(), reduction="mean")
                loss_aux1 = F.mse_loss(text_pred.float(), target.float(), reduction="mean")
                loss_aux2 = cal_cos(encoder_hidden_states, img_state, cos)
                loss_aux3 = F.mse_loss(img_pred.float(), target_noconcept.float(), reduction="mean")
                loss = main_loss + 0.01*loss_aux1 + 0.001*loss_aux2 + 0.01 * loss_aux3
            ...
            logs = {"loss": loss.detach().item(), "lr": lr_scheduler.get_last_lr()[0],
                    "main_loss": loss.detach().item(),
                    "text_rec_loss": loss_aux1.detach().item(),
                    "contrasive_loss": loss_aux2.detach().item(),
                    "img_rec_loss": loss_aux3.detach().item()}
