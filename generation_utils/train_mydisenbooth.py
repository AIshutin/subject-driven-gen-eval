...
These are LoRA adaption weights for {base_model}. The weights were trained on {prompt} using [Disenbooth](https://disenbooth.github.io/). You can find some example images in the following. \n
{img_str}
    ...
    with torch.cuda.amp.autocast():
        for _ in range(args.num_validation_images):
            for prompt in args.validation_prompt:
                image = pipeline(prompt=prompt, num_inference_steps=25,
...
def parse_args(input_args=None):
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.01,
        help="Weak denoising loss weight. Check out DisenBooth paper for details."
    )
    parser.add_argument(
        "--contrasive_loss_weight",
        type=float,
        default=0.001,
        help="Contrasive loss weight. Check out DisenBooth paper for details."
    )
    parser.add_argument(
        "--adapter_dim",
        type=int,
        default=1024,
        help="Input dimension of the image encoder adapter"
    )
    parser.add_argument(
        "--checkpointing_steps",
        type=int,
...
class DisenBoothDataset(Dataset):
    """
    ...
    def __getitem__(self, index):
        ...
        example['original_subject_images'] = instance_image
        ...
        if self.class_data_root:
            ...
            example['original_class_images'] = class_image
...
class DisenBoothAdapter(nn.Module):
    def __init__(self, n_feats=1024):
        super().__init__()
        self.mask = nn.Parameter(torch.zeros(size=(n_feats,)))
...
def main(args):
    ...
    adapter = DisenBoothAdapter(args.adapter_dim)
    ...
    def save_model_hook(models, weights, output_dir):
        if accelerator.is_main_process:
            ...
            for model in models:
                if isinstance(model, DisenBoothAdapter):
    ...
    def load_model_hook(models, input_dir):
        ...
        while len(models) > 0:
            ...
            if isinstance(model, type(unwrap_model(unet))):
                unet_ = model
            elif isinstance(model, DisenBoothAdapter):
                model.load_state_dict(
                    torch.load(os.path.join(input_dir, 'adapter.pt'))
                )
            elif isinstance(model, type(unwrap_model(text_encoder))):
                text_encoder_ = model
            else:
    ...
    params_to_optimize += list(adapter.parameters())
    ...
    train_dataset = DisenBoothDataset(
    ...
    if args.train_text_encoder:
        unet, adapter, text_encoder, optimizer, train_dataloader, lr_scheduler = accelerator.prepare(
            unet, adapter, text_encoder, optimizer, train_dataloader, lr_scheduler
        )
    else:
    ...
    if accelerator.is_main_process:
        ...
        accelerator.init_trackers("disenbooth", config=tracker_config)
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            with accelerator.accumulate(unet):
                ...
                if args.pre_compute_text_embeddings:
                    subject_embeddings = batch["input_ids"]
                else:
                    subject_embeddings = encode_prompt(
                        text_encoder,
                        batch["input_ids"],
                ...
                image_encoder_hidden_states = image_encoder.encode_image(batch['subject_images4ie'] \
                                                           .to(device=accelerator.device, dtype=weight_dtype))
                visual_embeddings = adapter(image_encoder_hidden_states)
                # Predict the noise residual
                model_pred_wo_visual = unet(
                    noisy_model_input,
                    timesteps,
                    subject_embeddings,
                ...
                model_pred_with_visual = unet(
                    noisy_model_input, 
                    timesteps, 
                    subject_embeddings + visual_embeddings, 
                    class_labels=class_labels, 
                    return_dict=False
                )[0]
                ...
                if model_pred_wo_visual.shape[1] == 6:
                    model_pred_wo_visual, _ = torch.chunk(model_pred_wo_visual, 2, dim=1)
                    model_pred_with_visual, _ = torch.chunk(model_pred_with_visual, 2, dim=1)
                ...
                '''
                main_loss = F.mse_loss(model_pred_with_visual.float(), target.float(), reduction="mean")
            ...
            logs = {"loss": loss.detach().item(), 
                    ...
                    "weak_loss": weak_loss.detach().item(),
                    "contrasive_loss": contrasive_loss.detach().item(),
